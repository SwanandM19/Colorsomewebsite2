...
const productIndex = products.findIndex(p => cleanStr(p.slug) === cleanStr(slug));
  ...
  // Build pack sizes: use product's own array if present, otherwise detect unit types from product details
  const isKgProduct = 
    (product as any).unitType === 'Kg' || 
    product.name.toLowerCase().includes('putty') || 
    product.description.toLowerCase().includes('cement') ||
    product.finish?.toLowerCase().includes('kg');

  const packSizes: string[] = (product as any).packSizes ?? product.packSizes ?? (
    isKgProduct 
